...
'''
    ...
    for i in range(length):
        bit = int(binary_string[length - 1 - i])  # Convert character to integer ('0' or '1')
        decimal += bit * (2 ** i)
    return decimal
...
'''
    ...
    ciphered_message = []
    last_added_key = None  # Variable to store the last added key
    
    for char in message:
        ciphered_letter = encoder_dict.get(char)
        
        # Check if last added key exists and if it matches the current key
        if last_added_key and last_added_key == ciphered_letter[0]:
            ciphered_message.append("_")
        
        ciphered_message.append(ciphered_letter)
        last_added_key = ciphered_letter[0]  # Update last added key
...
def telephone_decipher(telephone_string):
    ...
    while i < len(telephone_string):
        ...
        char = telephone_string[i]
        if char == '_':
            i += 1
            continue
        
        start_index = i
        while i < len(telephone_string) and telephone_string[i] == char:
            current_sequence += char
            i += 1
        
        if current_sequence in decipher_dict:
            decipher_message.append(decipher_dict[current_sequence])
